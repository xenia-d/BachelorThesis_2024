...
@final
class IROF_depth(Metric[List[float]]):
    """
    ...
    def evaluate_instance(
        ...
        """
        # Predict on x.
        print("yay we evaluate instance now")
    ...
    def plot_aoc_curves(self, instance_scores: List[List[float]]) -> None:

        plt.figure(figsize=(10, 6))

        for i, scores in enumerate(instance_scores):
            plt.plot(scores, label=f'Image {i + 1}')

        plt.xlabel('Number of Features Removed')
        plt.ylabel('Prediction Score')
        plt.title('AOC Curves for Each Image')
        plt.legend()
        plt.grid(True)
        plt.show()
