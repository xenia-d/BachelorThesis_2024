...
OUT_CHANNEL_DIM = 0
IN_CHANNEL_DIM = 1 # was set to 1 before
...
def prune_filters(norm_layer_name, keep_mask, weights, grouped_weight_names, prec_layers, succ_layers):
    ...
    for weight_name in grouped_weight_names[norm_layer_name]:
        if weight_name.endswith(BIAS_POSTFIX) or weight_name.endswith(WEIGHT_POSTFIX):
            weights[weight_name][~keep_mask] = 0

    # 2. Prune target succeeding conv/linear layers
    successor_layers = succ_layers[norm_layer_name]
    if not isinstance(successor_layers, list):
        successor_layers = [successor_layers]

    for prune_layer_name in successor_layers:
        if prune_layer_name is None:
            continue

        for weight_name in grouped_weight_names[prune_layer_name]:
            if weight_name.endswith(WEIGHT_POSTFIX) and weights[weight_name].ndim > 1:
                weights[weight_name][:~keep_mask.sum(), :, :, :] = 0
            elif weights[weight_name].ndim <= 1 and (weight_name.endswith(BIAS_POSTFIX) or weight_name.endswith(WEIGHT_POSTFIX)):
                weights[weight_name][:~keep_mask.sum()] = 0

    # 3. Prune target preceding conv/linear layers
    preceeding_layers = prec_layers[norm_layer_name]
    if not isinstance(preceeding_layers, list):
        preceeding_layers = [preceeding_layers]

    for prune_layer_name in preceeding_layers:
        if prune_layer_name is None:
            continue
...
def prune_filters(norm_layer_name, keep_mask, weights, grouped_weight_names, prec_layers, succ_layers):
    ...
    for prune_layer_name in successor_layers:
        ...
        for weight_name in grouped_weight_names[prune_layer_name]:
            if weight_name.endswith(WEIGHT_POSTFIX) and weights[weight_name].ndim > 1:
                keep_indices = keep_indices[keep_indices < weights[weight_name].shape[1]]
                print(weights[weight_name].shape)

                weights[weight_name] = weights[weight_name].index_select(IN_CHANNEL_DIM, keep_indices)

                print("after pruning", weights[weight_name].shape)
            elif weights[weight_name].ndim <= 1 and (weight_name.endswith(BIAS_POSTFIX) or weight_name.endswith(WEIGHT_POSTFIX)):
                keep_indices = keep_indices[keep_indices < weights[weight_name].shape[0]]
...
def uniform_scaling(weights, prune_ratio, prec_layers, succ_layers):
    ...
    for norm_layer_name in norm_layer_names:
        ...
        print("newewew prune th", prune_th)
...
def convert_to_batch_norm_format(state_dict):
    representations = []

    for key in state_dict.keys():
        components = key.split('.')
        if len(components) >= 4:
            layer_index = components[-3]
            parameter = components[-1]
            representation = f'batch_norm(%input.{layer_index},.{parameter})'
...
def prune(model, input_shape, prune_method=network_slimming, prune_ratio = 0, channel_select=True):
    """prune a model
    ...
    prunable, bn_names, prec_layers, succ_layers = get_pruning_layers(model, input_shape)
    ...
    return pruned_model.to(src_device), pruned_weights #, eliminated_layers
