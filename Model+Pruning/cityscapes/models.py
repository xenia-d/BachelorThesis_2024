                    ...
                    if layer_name == name:
                        ...
                        for child_name, child_module in self.named_modules():
                            if isinstance(child_module, nn.BatchNorm2d) and child_name == name.replace('conv', 'bn'):
                                ...
                                child_module.num_features = filter_size[1]
        ...
        for name, module in self.named_modules():
            for layer_name, filter_size in custom_list:
                if name.endswith(layer_name):  
                    ...
                    for child_name, child_module in self.named_modules():
                        if child_name.endswith(layer_name[:-2] + ".1"):
                            print("child module:", child_name, "layer name:", layer_name[:-2] + ".1")
                            if isinstance(child_module, torch.nn.BatchNorm2d):  
                                child_module.num_features = filter_size[0]
                                # Ensure that the running statistics are correctly updated
                                if hasattr(child_module, 'running_mean') and hasattr(child_module, 'running_var'):
                                    with torch.no_grad():
                                        # Update running statistics by resizing or padding if necessary
                                        current_size_mean = len(child_module.running_mean)
                                        current_size_var = len(child_module.running_var)
                                        print("current size mean", current_size_mean)
                                        new_size = filter_size[0]
                                        if current_size_mean != new_size:
                                            # print("new size: ", new_size)
                                            # child_module.running_mean = nn.Parameter(child_module.running_mean[:new_size])
                                            # child_module.running_var = nn.Parameter(child_module.running_var[:new_size])
                                            child_module.running_mean = child_module.running_mean[:new_size]
                                            child_module.running_var = child_module.running_var[:new_size]
                                            print(f"{name}.running_mean: {child_module.running_mean.size()}")
                                            print(f"{name}.running_var: {child_module.running_var.size()}")
                                            print()

        # Print the updated running statistics
        print("Batch Normalization Running Statistics:")
        for name, module in self.named_modules():
            if isinstance(module, torch.nn.BatchNorm2d):
                print(f"{name}.running_mean: {module.running_mean.size()}")
                print(f"{name}.running_var: {module.running_var.size()}")
...
class SegNetSplit(nn.Module):
    ...
    def forward(self, x):
        ...
        return [t1_pred, t2_pred], g_decoder[i][
            1
                ...
                for layer_name, filter_size in custom_list:
                    if layer_name == name:
                        ...
                        for child_name, child_module in self.named_modules():
                            if isinstance(child_module, torch.nn.BatchNorm2d):
                                # Update the number of features for the batch normalization layer
                                print("child:", child_module)
                                child_module.num_features = filter_size[1]
                                print("new child:", child_module)
